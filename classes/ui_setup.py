...
class main_window(Tk):
    def __init__(self, ):
        ...
        # Variables for chart-type selectors, check buttons
        ...
        # fixed: create a drop-down list
        self.drop_down = ttk.Combobox(self, width=37, textvariable=self.combobox_variables1,
                                      values=[element for element in list(self.df.keys()) if element not in ['incident_key','OCCUR_DATE','OCCUR_TIME']])

        # fixed: create several buttons
        ...
        # fixed: create several check-buttons for specifying the type of charts
        # Area chart
    ...
    def plot_chart(self):
        try:
            global RUN_TIME
            p1 = self.combobox_variables1.get().upper()
            print(p1)
